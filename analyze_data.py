...
def convert_to_datapoints(data_list):
    return [
        DataPoint(d["n"], d["capacity"], d["time"], d["accuracy"]) for d in data_list
    ]
...
def write_results_to_report(f, capacity, scenario, algorithm_results, fig_nr):
    ...
    eps = 1e-8
    f.write(
        f"% Results for capacity: {capacity} - scenario: {scenario} (figures {fig_nr} - {fig_nr + 2})\n\n"
    )
    ...
    problem_sizes = [
        (result.n, result.capacity) for result in algorithm_results[algorithms[0]]
    ]

    # exact methods time
    f.write("\\centering\n")
    f.write(
        "\\caption{Execution Time Comparison (Exact Methods) - "
        + f"Capacity: {capacity}, Scenario: {scenario}"
        + "}\n"
    )
    f.write("\\label{tab:time_exact_" + f"cap{capacity}_" + f"{scenario}" + "}\n")
    ...
    for i, size in enumerate(problem_sizes):
        ...
        for algo in exact_methods:
            ...
            if abs(result.time - best_times_exact[i]) < eps:
                ...
                row.append(
                    f"{result.time:.6f}" + "s" if result.time != math.inf else "inf"
                )
        f.write(" & ".join(row) + " \\\\\n")
    ...
    f.write("\\centering\n")
    f.write(
        "\\caption{Execution Time Comparison (Approximate Methods) - "
        + f"Capacity: {capacity}, Scenario: {scenario}"
        + "}\n"
    )
    f.write("\\label{tab:time_approx_" + f"cap{capacity}_" + f"{scenario}" + "}\n")
    ...
    f.write("\\label{tab:accuracy_" + f"cap{capacity}_" + f"{scenario}" + "}\n")
...
def plot_time_comparison(algorithm_results, capacity, scenario, fig_nr):
    ...
    labels = {
        ...
        "dp_bu": "bottom up dp",
        "bb": "branch and bound",
        "greedy_r": "greedy - ratio",
        "greedy_s": "greedy - stats",
        "greedy_v": "greedy - value",
        "greedy_w": "greedy - weight",
    ...
    plt.savefig(
        f"tests/out/plots/{fig_nr}_{capacity}_{scenario}_time_comparison.svg",
        format="svg",
        bbox_inches="tight",
        dpi=1200,
    )
...
def plot_accuracy_comparison(algorithm_results, capacity, scenario, fig_nr):
    ...
    plt.savefig(
        f"tests/out/plots/{fig_nr + 1}_{capacity}_{scenario}_accuracy_comparison.svg",
        format="svg",
        bbox_inches="tight",
        dpi=1200,
    )
...
def plot_efficiency_comparison(algorithm_results, capacity, scenario, fig_nr):
    ...
    plt.savefig(
        f"tests/out/plots/{fig_nr + 2}_{capacity}_{scenario}_efficiency_comparison.svg",
        format="svg",
        bbox_inches="tight",
        dpi=1200,
    )
...
def analyze_algorithms(
    data_path="tests/out/data.json", report_path="tests/out/report.txt"
):
        ...
        for capacity in capacities:
            ...
            for scenario in scenarios:
                ...
                write_results_to_report(
                    f, str_capacity, scenario, algorithm_results, fig_nr
                )
                plot_time_comparison(algorithm_results, str_capacity, scenario, fig_nr)
                plot_accuracy_comparison(
                    algorithm_results, str_capacity, scenario, fig_nr
                )
